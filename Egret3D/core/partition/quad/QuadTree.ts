...
module egret3d {
    ...
    export class QuadTree {
        private _cells: Array<QuadTreeCell>;		// 所有节点
        private _rootCell: QuadTreeCell;			// 根节点
        private _quadNodes: Array<IQuadNode>;
        private _aabb: QuadAABB;

        private _cellsToTest: Array<number>;			// 碰撞检测用数组
        ...
        // 构建四叉树
        public buildQuadTree(maxTrianglesPerCell: number, minCellSize: number): void {
            this._aabb.clear();
            ...
            for (var node of this._quadNodes) {
                if (node.isTriangle) {
                    ...
                    this._aabb.setContainRect(node.aabb.minPosX, node.aabb.minPosY, node.aabb.maxPosX, node.aabb.minPosY);
                }
            }
            ...
            while (cellsToProcess.length != 0) {
                ...
                if (this._cells[cellIndex].nodeIndices.length <= maxTrianglesPerCell
        ...
        // 创建子节点的AABox
        private createAABox(aabb: QuadAABB, _id: number): QuadAABB {
        ...
        private doesNodeIntersectCell(node: IQuadNode, cell: QuadTreeCell): Boolean {
            ...
            var points: Array<Vector3D> = box.points;
            var p1: Vector3D = points[0];
            var p2: Vector3D = points[1];
            var p3: Vector3D = points[2];
        ...
        // 判断点在三角型中
        private pointInTriangle(x: number, y: number, triP1: Vector3D, triP2: Vector3D, triP3: Vector3D): boolean {
