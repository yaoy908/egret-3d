module egret3d {
    ...
    export class HDRParser2 {
        ...
        public static parseHDR(buffer): DDSTexture {
            ...
            for (var i = 0; i < 20; i++) {
                var line = readLine();
                var match;
                if (match = line.match(HDRParser2.radiancePattern)) {
                }
                else if (match = line.match(HDRParser2.formatPattern)) {
                    rle = true;
                }
                else if (match = line.match(HDRParser2.exposurePattern)) {
                    exposure = Number(match[1]);
                }
                else if (match = line.match(HDRParser2.commentPattern)) {
                }
                else if (match = line.match(HDRParser2.widthHeightPattern)) {
